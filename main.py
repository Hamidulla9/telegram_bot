...
async def get_inn(
    ...
    if (
        ...
        or len(message.text) != 9
    ):

        m = await message.answer(
            "⚠️ Ошибка! Введите 9 цифр:"
        )
...
async def get_product(
    ...
    m = await message.answer(
        f"Введите количество для {message.text}:",
...
async def finish_order(
    ...
    order_info = {
        "Дата": now_str,
        "ID_Заказа": f"d0_{current_num}",
        "Агент": message.from_user.full_name,
        "Организация": data["company"],
        "ИНН": data["inn"],
        "Товар": data["product"],
        "Количество": quantity,
        "Цена_ед": PRICES[data["product"]],
        "Итого": total,
        "Локация": data["geo_url"]
    }
    ...
    report = (
        ...
        f"{data['company']}\n"
        f"🆔 <b>ИНН:</b> "
        f"<code>{data['inn']}</code>\n"
        f"📍 <a href='{data['geo_url']}'>"
        f"Локация</a>\n"
        f"🕒 <b>Время:</b> "
        f"{now_str}\n"
        f"━━━━━━━━━━━━━━━━━━\n"
